...
const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  if (!hasPermission('dashboard', 'canView')) {
    return (
      <div className="bg-white p-8 rounded-xl border shadow-sm text-center">
        ...
        <p className="text-gray-600">אין לך הרשאות לצפות בלוח הבקרה.</p>
      </div>
    );
  }
  ...
  const stats = [
    { label: 'לקוחות', value: data.customers.length, icon: <UsersIcon className="text-blue-500" />, bg: 'bg-blue-50', show: hasPermission('customers', 'canView') },
    { label: 'ביקורות פתוחות', value: techInspections.filter(i => i.status !== InspectionStatus.CLOSED).length, icon: <ClipboardCheck className="text-orange-500" />, bg: 'bg-orange-50', show: hasPermission('inspections', 'canView') },
    { label: 'אישורים שהונפקו', value: data.certificates.filter(c => c.status === 'Issued').length, icon: <FileText className="text-green-500" />, bg: 'bg-green-50', show: hasPermission('certificates', 'canView') },
    { label: 'ביקורות באיחור', value: 0, icon: <AlertCircle className="text-red-500" />, bg: 'bg-red-50', show: hasPermission('inspections', 'canView') },
  ].filter(s => s.show);
